...
| FEATURE_PHASE_PLAN revision r3 — commit PR (no completed record) | B |
...
The feature phase comprises **31 review objects** in total (FEATURE_PHASE_PLAN §4c: the
plan document, revisions r1 and r2 at two objects each and r3 at one, §GMF-000R, and
the nine GMF tickets at 2 + 3 + 2 + 2 + 3 + 3 + 3 + 3 + 3). Deployed
