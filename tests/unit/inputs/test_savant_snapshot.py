...
both directions: the 2023-2026 four-season window (D-171, PO) covers Sutter
Health Park on its two played seasons, so every Savant row matches a
reference venue and every reference venue has its Savant row.
"""
...
SNAPSHOT = REPO_ROOT / "data" / "savant_park_factors_2023-2026.csv"
...
def test_the_snapshot_bytes_match_the_certified_digest() -> None:
    ...
    assert len(SNAPSHOT.read_bytes()) == 7506


def test_the_provenance_record_sits_beside_the_snapshot_and_pins_it() -> None:
    text = PROVENANCE.read_text(encoding="utf-8")
    assert PINNED_SHA256 in text
    assert "2026-09-01" in text  # the scripted pulls' date (D-171)
    assert "baseballsavant.mlb.com" in text  # the source URL, documented not fetched
    assert "scripted pulls" in text  # the D-171 acquisition, stated honestly
    assert "plate-appearance-weighted" in text  # the derivation rule, documented
...
def test_the_window_is_uniform_and_four_season() -> None:
    """D-171 (PO): the 2023-2026 window the PO asked for. Savant publishes no
    four-year board, so the snapshot is derived from the four single-year
    boards and the meta fields are the derived window's own descriptors."""
    for row in _rows():
        assert row["key_year"] == "2026"  # the window's end season
        assert row["key_num_years_rolling"] == "4"  # derived-window descriptor (D-171)
        assert row["key_is_year_rolling"] == "-1"  # derived-window descriptor (D-171)
        assert row["year_range"] == "2023-2026"


def test_every_savant_row_joins_a_reference_venue() -> None:
    """Total in both directions since D-166 closed the Athletics' gap, held
    through the D-171 window change: the derived file lists the same thirty
    current venues."""
...
def test_n_pa_is_positive_everywhere_and_spans_the_stated_range() -> None:
    ...
    assert values[0] == 13438  # Sutter Health Park, LHB — two played seasons
    assert values[-1] == 44096  # Daikin Park, RHB
    assert all(value > 0 for value in values)


def test_the_athletics_gap_is_closed_with_real_numbers() -> None:
    """D-171 (PO): Sutter Health Park's factors are the documented blend of
    its two played seasons — 119 (LHB) / 118 (RHB) — joined on Savant venue
    id 2529, never filled in from a league average."""
    ...
    assert {row["key_bat_side"]: row["index_hr"] for row in rows} == {"L": "119", "R": "118"}
...
def test_a_park_factor_carries_its_sample_beside_the_value() -> None:
    factor = ParkFactor(factor=Decimal("119"), handedness=Handedness.LEFT, plate_appearances=13438)
    assert factor.plate_appearances == 13438  # D-014: beside, never fused
