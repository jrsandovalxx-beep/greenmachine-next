...
def test_the_product_consumes_the_home_run_factor_not_the_headline() -> None:
    ...
    # D-172's snapshot: Fenway LHB index_hr = 85 over 24,390 PA on the
    # published 2024-2026 three-season board.
    assert lhb.factor == Decimal("85")
    assert lhb.plate_appearances == 24390
...
def test_the_athletics_are_covered_since_d166s_gap_closure() -> None:
    """D-166 closed the Sutter Health Park gap; D-172 (PO) holds it on
    Savant's published 2025-2026 two-season board — the only window Savant
    publishes for a park that opened in 2025 — and the Athletics read real
    factors like every club."""
    factors = read_factors()
    ...
    assert fields[Handedness.LEFT].value.factor == Decimal("120")
    assert fields[Handedness.LEFT].value.plate_appearances == 13865
...
def test_the_source_record_carries_the_pinned_provenance() -> None:
    ...
    assert PROVENANCE.export_date.isoformat() == "2026-09-02"  # the D-172 scripted pulls
...
def test_the_basis_statement_says_rolling_window_and_export_date() -> None:
    ...
    assert "three-season" in statement
    assert "published" in statement  # no derivation is left to name
    assert "2025-2026" in statement  # the Sutter exception is named on screen
    assert "2026-09-02" in statement
