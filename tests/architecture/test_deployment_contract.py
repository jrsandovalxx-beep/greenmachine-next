...
def test_the_entrypoint_evicts_a_preloaded_stale_bootstrap(tmp_path: Path) -> None:
    ...
    before the pre-import; eviction is the guard that remains. D-158/D-159
    add the three properties the host's PicklingError demanded: the
    eviction runs at most once per DEPLOY EPOCH (the host's process
    outlives deploys, so a process-wide flag disables the eviction the
    next deploy needs; an every-rerun check evicted a half-initialized
    package mid-import), a half-initialized module is an import in
    flight — never a stale serve — and a real eviction schedules a full
    st-cache clear because cached objects hold the old set's classes."""
    import os
    import py_compile
    import sys
    ...
    payload = (
        ...
        " != expected for r in ('deploy_bootstrap', 'greenmachine') if r in sys.modules):\n"
        ...
        # marker fires the eviction (popping mid-import breaks importlib's
        # completion re-registration). The old-epoch bootstrap goes.
